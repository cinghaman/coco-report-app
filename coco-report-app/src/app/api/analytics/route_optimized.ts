...
export async function POST(request: NextRequest) {
  try {
    ...
    const totalWithdrawals = dailyAnalyticsData?.reduce((sum, day) => sum + (Number(day.withdrawals) || 0), 0) || 0

    // Calculate averages
    const totalDays = Math.max(1, Math.ceil((new Date(endDate).getTime() - new Date(startDate).getTime()) / (1000 * 60 * 60 * 24)))
    const averageDailySales = totalGrossSales / totalDays
    const averageDailyWithdrawals = totalWithdrawals / totalDays
    ...
    dailyAnalyticsData?.forEach(day => {
